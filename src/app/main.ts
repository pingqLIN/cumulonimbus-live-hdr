...
function resize(): void {
  ...
  const fallback = orientation === "landscape" ? { width: 960, height: 540 } : { width: 540, height: 960 };
  const width = readNumber(query, ["simWidth", "width"], Math.round(rect.width || fallback.width), 2, 7680);
...
function resolveOptions(params: URLSearchParams): RaymarchCloudOptions {
  ...
  return {
    ...preset,
    seed: readNumber(params, ["seed"], preset.seed ?? 574, 1, Number.MAX_SAFE_INTEGER),
    time: readNumber(params, ["time"], preset.time ?? 0, 0, 1_000_000),
    ...
    windShear: readNumber(params, ["windShear", "shear"], preset.windShear ?? 0.7, 0, 1),
    sunIntensity: readNumber(params, ["sun", "sunIntensity"], preset.sunIntensity ?? 4.6, 0, 10),
    ambientIntensity: readNumber(
      params,
      ["ambient", "amb", "ambientIntensity"],
      preset.ambientIntensity ?? 0.75,
      0,
      2
    ),
    ...
    sunViewerAngle: readNumber(
      ...
      preset.sunViewerAngle ?? 25,
      -180,
      180
    ),
    photographicStyle: readBoolean(params, ["photographic", "photo"], preset.photographicStyle ?? false),
    lightPreset: resolveLightPreset(params.get("light") ?? params.get("lighting")) ?? preset.lightPreset,
    skyMode: resolveSkyMode(params.get("sky") ?? params.get("background") ?? params.get("bg")) ?? preset.skyMode,
    ...
    maxPixels: readNumber(params, ["maxPixels"], 1920 * 1080, 128 * 128, 3840 * 2160)
  };
}
...
function readTransparentBackground(params: URLSearchParams): boolean {
  ...
  return background === "0" || background === "false" || background === "none" || background === "transparent";
}

function readBoolean(params: URLSearchParams, names: readonly string[], fallback: boolean): boolean {
